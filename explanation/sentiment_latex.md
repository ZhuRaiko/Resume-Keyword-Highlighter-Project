% sentiment_latex.md — readable LaTeX layout with highlighted comments

\section*{sentiment.py}

\subsection*{Overview}
\textbf{Purpose:} Lightweight TextBlob-based sentiment helpers for contextual heuristics.

\textbf{Role in system:} Provides polarity and boolean checks used to adjust highlighting and scoring confidence in ambiguous contexts.

\subsection*{Notes on this LaTeX view}
The descriptive text above is plain black. The full source is shown in a `lstlisting` block below; compile with the `listings` and `xcolor` packages and set `commentstyle=\color{green}` so inline comments render green while LaTeX text remains black.
