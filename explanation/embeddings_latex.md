% embeddings_latex.md — readable LaTeX layout with highlighted comments

\section*{embeddings.py}

\subsection*{Overview}
\textbf{Purpose:} Load spaCy and SentenceTransformer models and provide a helper to encode a single sentence.

\textbf{Role in system:} Supplies `nlp` and `bert_model` used across scoring, highlighting, and embedding-based components.

\subsection*{Notes on this LaTeX view}
The module-level explanatory text above is plain (black). The code is shown below inside a `lstlisting` block; when compiling include `listings` and `xcolor` and set `commentstyle=\color{green}` so inline comments render green while surrounding LaTeX text remains black.
