% scoring_latex.md — readable LaTeX layout with highlighted comments

\section*{scoring.py}

\subsection*{Overview}
\textbf{Purpose:} Score resume sentences for self-promotion using a KNN classifier and a
set of heuristic boosts (metrics, achievement wording, bullets, sentiment, action verbs).

\textbf{Role in system:} Called from the main application to analyze and rank sentences; relies
on BERT embeddings and returns per-sentence probabilities used to compute an overall score.

\subsection*{Notes on this LaTeX view}
The code is shown inside a \texttt{lstlisting} environment. Comments are styled
green for visual separation; explanatory text above and between blocks remains
standard black text for readability. To compile this fragment, include the
`listings` and `xcolor` packages and the appropriate style definitions. Example:
...
def analyze_sentences(nlp, knn_model, _bert_model, text: str, action_verbs: list, file_type=None):
    ...
    try:
        sentences = []
        protected_terms = {'.net', 'node.js', 'next.js', 'express.js', 'socket.io', 'd3.js', 'vue.js',
