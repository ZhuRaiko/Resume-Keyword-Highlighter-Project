...
# --- Lightweight, format-robust helpers ---
...
def fallback_segment(text: str) -> list:
    ...
    Splits by newlines, bullets, and terminal punctuation while avoiding over-splitting acronyms like .NET.
    Returns a list of candidate sentences.
    """
    if not text:
        return []
    # First split by explicit line breaks or bullets
    ...
    for p in parts:
        ...
        if not p:
            continue
        # Secondary split by punctuation, protecting known tech tokens with dots
        # Keep tokens like .NET, Node.js, Next.js intact
        protected = {".net", "node.js", "next.js", "express.js", "socket.io"}
        # Temporarily replace dots in protected tokens
        tmp = p
        for tok in protected:
            tmp = re.sub(re.escape(tok), tok.replace('.', '•DOT•'), tmp, flags=re.IGNORECASE)
        ...
        for L in range(min(max_kw_len, n - i), 0, -1):
            ...
            if label in ('SOFT', 'RECRUITER'):
                ...
                if len(window) == 1:
                    ...
                    if tok.pos_ == 'ADJ' and tok.head != tok and tok.head.pos_ == 'NOUN' and tok.i < tok.head.i:
                        # When relaxed soft-skill mode is ON, allow adjectival uses to highlight
                    ...
                    if not skip_match:
                        for child in last_tok.children:
                            if child.i > last_tok.i and child not in window and child.dep_ in ('amod', 'compound', 'nmod'):
                                # In relaxed soft-skill mode, allow broader phrases
                                if not (label == 'SOFT' and RELAX_SOFT):
                                    skip_match = True
                                break
                if skip_match:
            ...
            # build html
            if RENDER_LEGACY:
                # Simpler, legacy-style rendering without padding or rounded corners
...
with st.sidebar:
    ...
    RENDER_LEGACY = st.checkbox("Legacy HTML rendering", value=RENDER_LEGACY,
...
if text:
    # Light normalization for robustness across formats
    ...
    # Offer optional fallback segmentation for tricky formats
    use_fallback_seg = st.checkbox("Use fallback segmentation", value=False,
                                   help="If sentences look broken, try a gentler split by lines/bullets.")

    # Run analysis (spaCy first, fallback if requested)
    if use_fallback_seg:
        fb_sents = fallback_segment(text)
        if fb_sents:
            # Apply full scoring logic to fallback segments (not just base KNN)
            scored = []
            for s in fb_sents:
                s_lower = s.lower()
                base = self_promotion_score(s)
            ...
            results = scored
            avg_score = np.mean([s for _, s in results]) if results else 0.0
        else:
    ...
    # Display controls for sentence analysis
    col_limit1, col_limit2 = st.columns(2)
    with col_limit1:
        max_sents = st.slider("Max sentences to show", min_value=10, max_value=min(500, len(results)), 
                             value=min(100, len(results)), step=10,
                             help="Limit the number of sentences displayed below")
    with col_limit2:
        truncate_len = st.slider("Truncate sentence length", min_value=100, max_value=5000, value=500, step=100,
                                help="Long sentences will be trimmed for display only")
    
    if len(results) > max_sents:
        st.info(f"ℹ️ Showing top {max_sents} of {len(results)} sentences. Adjust slider to see more.")
    
    # Apply display limits
    to_show = sorted(results, key=lambda x: x[1], reverse=True)[:max_sents]
    for sent, score in to_show:
        if len(sent) > truncate_len:
            sent = sent[:truncate_len] + "…"
